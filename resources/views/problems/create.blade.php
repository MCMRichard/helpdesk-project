    ...
    <div class="container py-4">
        <div class="card shadow-sm">
            <div class="card-header bg-dark text-white">
                <h1 class="h4 mb-0">Log New Problem</h1>
            </div>
            <div class="card-body">
                @if (session('success'))
                ...
                <form method="POST" action="{{ route('problems.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select id="caller_id" name="caller_id" class="form-control @error('caller_id') is-invalid @enderror" required>
                            ...
                            @foreach ($callers as $caller)
                                <option value="{{ $caller->caller_id }}" {{ old('caller_id') == $caller->caller_id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="problem_type_id" name="problem_type_id" class="form-control @error('problem_type_id') is-invalid @enderror" required>
                            ...
                            @foreach ($problemTypes as $type)
                                <option value="{{ $type->problem_type_id }}" {{ old('problem_type_id') == $type->problem_type_id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="equipment_serial" name="equipment_serial" class="form-control @error('equipment_serial') is-invalid @enderror">
                            ...
                            @foreach ($equipment as $item)
                                <option value="{{ $item->serial_number }}" {{ old('equipment_serial') == $item->serial_number ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="software_id" name="software_id" class="form-control @error('software_id') is-invalid @enderror">
                            ...
                            @foreach ($software as $soft)
                                <option value="{{ $soft->software_id }}" {{ old('software_id') == $soft->software_id ? 'selected' : '' }}>
                    ...
                    <!-- Submit -->
                    <button type="submit" class="btn btn-primary">Log Problem</button>
